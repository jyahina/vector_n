...
#include "standart_vector_n.hpp"
#include "standart_deque_n.hpp"
#include <omp.h>
#include <ctime>
#include <cassert>
...
void testAllVector2d()
{
	int nx = 20, ny = 10;

	vector_n<int, 2> vec_n(nx, ny);
	standart_vector_n<int, 2> newVector2d(nx, ny);
	standart_deque_n<int, 2> newDeq2d(nx, ny);
	std::vector<vec_int> simpleVector2d(nx, vec_int(ny));
	...
	for (int i = 0, n = 0; i < nx; ++i)
		for (int j = 0; j < ny; ++j)
		{
			...
			newVector2d(i, j) = n;
			newDeq2d(i, j) = n;
			simpleVector2d[i][j] = n;

			++n;
			if (vec_n(i, j) != simpleVector2d[i][j]) std::cout << "testAllVector2d FAIL ";
			if (newVector2d(i, j) != simpleVector2d[i][j]) std::cout << "testAllVector2d FAIL ";
			if (newDeq2d(i, j) != simpleVector2d[i][j]) std::cout << "testAllVector2d FAIL ";
		}

	std::cout << "testAllVector2d OK ";
}
